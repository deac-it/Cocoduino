...
@synthesize bsdPath, name = serviceName, serviceType, readDelegate, writeDelegate;
...
{
	NSString *result= [NSString stringWithFormat:@"<%@: address: %p, name: %@, path: %@, serviceType: %@, fileHandle: %@, fileDescriptor: %d>", NSStringFromClass([self class]), self, self.name, self.bsdPath, self.serviceType, fileHandle, fileDescriptor];
	return result;
}
...
// Sean: why is O_NONBLOCK commented?  Do we want it or not?
