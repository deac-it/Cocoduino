...
- (void)updateLineNumbersForClipView:(NSClipView *)clipView checkWidth:(BOOL)checkWidth recolour:(BOOL)recolour
{
	...
	if (checkWidth == YES) {
		...
		if (widthOfStringInGutter >= ([[document valueForKey:@"gutterWidth"] integerValue] - 15)) { // Check if the gutterTextView has to be resized
			...
			[gutterScrollView setFrame:NSMakeRect(0, 0, gutterWidth, currentViewBounds.size.height)];
		}
		else if (widthOfStringInGutter < ([[document valueForKey:@"gutterWidth"] integerValue] - 20)) { // Check if the gutterTextView has to be resized
            [document setValue:[NSNumber numberWithInteger:widthOfStringInGutter + 20] forKey:@"gutterWidth"]; // Make it smaller than need be so it doesn't have to resized soon again
			if ([[document valueForKey:@"showLineNumberGutter"] boolValue] == YES) {
				gutterWidth = [[document valueForKey:@"gutterWidth"] integerValue];
			} else {
				gutterWidth = 0;
