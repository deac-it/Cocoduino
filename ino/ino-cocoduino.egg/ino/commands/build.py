...
sys.path.append(path.join(path.dirname(__file__),'../packages/Jinja2-2.6-py2.7.egg'))
...
class Build(Command):
    ...
    def _scan_dependencies(self, dir, lib_dirs, inc_flags):
        ...
        # Halt on make exception, also get the output
        try:
            output = subprocess.check_output(['make', '-f', makefile, 'all'])
        except subprocess.CalledProcessError as ex:
            raise Abort('Could not find some of the dependencies')
        
        # Print the output if it's different from "nothing to be done"
        if output.find('make: Nothing to be done') is -1:
            print output.strip()
        ...
        # libraries dirname and have a library name
        lib_re_template = ur'^(?P<libdir>{dirname}{slash}.*){slash}'
        # rules for matching dependencies in src are more stringent
        # we match only the src root, and not subdirectories
        src_re_template = ur'^(?P<libdir>{dirname}){slash}'
        # Used for splitting lines at non-escaped spaces
        split_re = re.compile(ur'(?<!\\)\s')
        
        def re_compile(expr, dirname):
            return re.compile(expr.format(dirname=re.escape(dirname),
                                          slash=re.escape(os.path.sep)))
        
        src_re   = re_compile(src_re_template, self.e.src_dir)
        lib_re   = re_compile(lib_re_template, self.e.lib_dir)
        dist_re  = re_compile(lib_re_template, self.e.arduino_libraries_dir)
        split_re = re.compile(split_re)
        
        used_libs = set()
        with open(output_filepath) as f:
            for line in f:
                includes = [ inc for inc in split_re.split(line) if len(inc) > 5 ]
                for inc in includes:
                    matches = src_re.findall(inc) +\
                        lib_re.findall(inc) +\
                        dist_re.findall(inc)
    ...
    def scan_dependencies(self):
        ...
        # Include local libs, distribution, then src
        lib_dirs = list_subdirs(self.e.lib_dir) +\
            list_subdirs(self.e.arduino_libraries_dir) +\
            [self.e.src_dir]
        inc_flags = self.inc_lib_flags(lib_dirs, recursive=True)
        ...
        self.e['cflags'].extend(self.inc_lib_flags(used_libs))
    
    def build(self):
        makefile = self.render_template('Makefile.jinja', 'Makefile')
        # Halt on build error
        ex = subprocess.call(['make', '-f', makefile, 'all'])
        if ex is not 0:
            raise Abort('Could not build the sketch')
